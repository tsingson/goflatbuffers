...
func TestVector_Array_Struct(t *testing.T) {
	...
	v1 := CreateVec3(flatbuffers.NewStruct(), 1.1, 1.1, 1.2, ColorGreen)
	v2 := CreateVec3(flatbuffers.NewStruct(), 1.2, 1.1, 1.2, ColorGreen)
	v3 := CreateVec3(flatbuffers.NewStruct(), 1.3, 1.1, 1.2, ColorGreen)

	vf := flatbuffers.NewVector().StructArray(0, []flatbuffers.VField{v1, v2, v3}...)
	buf := vf.FinishedBytes()
