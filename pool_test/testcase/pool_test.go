...
func TestRange(t *testing.T) {
	min := bytepool.nextLogBase2(1)
	max := bytepool.nextLogBase2(uint32(bytepool.MaxLength))
	if int(max) != len(bytepool.GlobalPool.pools)-1 {
		t.Errorf("expected %d pools, found %d", max, len(bytepool.GlobalPool.pools))
	}
	if min != 0 {
